...
const RunnerDevUrlCards: React.FC<RunnerDevUrlCardsProps> = ({
  variant = "full",
}) => {
  ...
  return (
    <div
      className={`grid grid-cols-1 ${
        isCompact
          ? "sm:grid-cols-3 gap-3 max-w-2xl"
          : "md:grid-cols-3 gap-4 max-w-4xl"
      } mx-auto`}
    >
      {cards.map((card, index) => (
        <a
          ...
          className={`inline-flex items-center ${padding} bg-gradient-to-r ${card.gradient} rounded-lg hover:shadow-lg transition-shadow`}
        >
          <card.icon className={`${iconSize} ${card.iconColor} mr-2`} />
          <div className="text-left">
            <div
              className={`${labelTextSize} text-gray-600 dark:text-gray-400`}
            >
              {isCompact ? card.shortLabel : card.label}
            </div>
            <code
              className={`${textSize} text-emerald-600 dark:text-emerald-400`}
            >
              {card.url}
            </code>
          </div>
