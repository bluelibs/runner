...
### Durable Step Cancellation Signals

Durable step callbacks now receive a live `{ signal: AbortSignal }` context so running work can cooperate with execution cancellation mid-step:

```ts
await ctx.step("ship-order", async ({ signal }) => {
  signal.throwIfAborted();
  return await shippingApi.createLabel(orderId, { signal });
});
```

- Works with abort-aware APIs such as `fetch(..., { signal })`
- Lets long-running loops bail out cleanly between chunks of work
- Replay stays deterministic: cached steps still return without re-running the callback

Cancellation is still cooperative. For actively running executions, `cancelExecution()` now first records the request and flips the step signal, then marks the execution terminal `cancelled` once the attempt exits.

---

### Cron Schedule Timezones

Recurring cron schedules now support an explicit `timezone` field:

```ts
await durable.ensureSchedule(reportTask, {}, {
  id: "daily-report",
  cron: "0 9 * * *",
...
- Uses IANA timezone identifiers such as `"UTC"` or `"America/New_York"`
- Keeps business-facing wall-clock schedules stable across DST transitions
- Applies to schedule creation and schedule updates

If `timezone` is omitted, cron schedules continue to use the process local timezone.
...
| Durable idempotent start | Repeated idempotent starts now behave as start-or-get for running or sleeping workflows; only pending/retrying executions are re-admitted |
