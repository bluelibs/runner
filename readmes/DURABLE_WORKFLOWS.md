...
Use your own cron/background job for retention:

```ts
const candidates = await durable.operator.fetchWorkflowsBefore(cutoff);
// Copy candidates to your archive of choice.
await durable.operator.deleteWorkflowsBefore(cutoff);
```

Only `completed`, `failed`, and `cancelled` workflows are returned/deleted by
default. `compensation_failed` stays operator-recoverable.
