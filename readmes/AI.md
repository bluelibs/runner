...
**Durable Workflows (Node-only):** For persistence and crash recovery, see `DURABLE_WORKFLOWS.md`. Includes `ctx.switch()` (replay-safe branching) and `describeFlow()` (static flow shape export) — see `DURABLE_WORKFLOWS_AI.md` for quick reference.
