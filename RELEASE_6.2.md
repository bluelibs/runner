...
const probability = check(0.5, Match.Range({ min: 0, max: 1, inclusive: false }));
...
```ts
await asyncContexts.execution.provide({ signal: controller.signal }, async () => {
...
- [ ] Document execution context lightweight mode (`frames: "off"`) in the
  observability chapter.
- [ ] Update error reference documentation to reflect the new shortened error ID
  format.
