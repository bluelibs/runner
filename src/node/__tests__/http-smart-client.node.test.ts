...
describe("createHttpSmartClient (unit)", () => {
  ...
  it("JSON: posts JSON and parses ok envelope", async () => {
    const spy = jest.spyOn(http, "request").mockImplementation((opts: any, cb: any) => {
      ...
      const im = asIncoming(res, { "content-type": "application/json; charset=utf-8" });
  ...
  it("JSON: invokes onRequest hook with headers", async () => {
    ...
    expect(out).toBe(1);
    expect(onRequest).toHaveBeenCalledWith(expect.objectContaining({ url: expect.any(String), headers: expect.any(Object) }));
  });
  ...
  it("multipart: detects File sentinel and returns JSON result", async () => {
    ...
    const input = {
      file: createNodeFile({ name: "a.txt" }, { stream: Readable.from("abc") }, "F1"),
    } as const;
  ...
  it("multipart (buffer): covers buffer branch in encoder", async () => {
    ...
    const input = {
      file: createNodeFile({ name: "a.bin", type: "application/octet-stream" }, { buffer: Buffer.from([1,2,3]) }, "F2"),
    } as const;
    const out = await client.task("upload", input as any);
    expect(out).toBe("BUF");
    expect(spy).toHaveBeenCalled();
  ...
  it("multipart (streaming response): returns Readable when server streams", async () => {
    ...
    const input = {
      file: createNodeFile({ name: "a.txt" }, { stream: Readable.from("x") }, "F3"),
    } as const;
    const out = (await client.task("upload", input as any)) as Readable;
    const buf = await new Promise<Buffer>((resolve, reject) => {
      const chunks: Buffer[] = [];
      out.on("data", (c: any) => chunks.push(Buffer.isBuffer(c) ? c : Buffer.from(String(c))));
      out.on("end", () => resolve(Buffer.concat(chunks as readonly Uint8Array[])));
      out.on("error", reject);
    });
  ...
  it("parseMaybeJsonResponse error path: rejects when JSON parsing fails", async () => {
    ...
    const input = {
      file: createNodeFile({ name: "a.txt" }, { stream: Readable.from("x") }, "F4"),
    } as const;
  ...
  it("uses https.request when baseUrl is https and includes auth header", async () => {
    const httpsReqSpy = jest.spyOn(require("https"), "request").mockImplementation((opts: any, cb: any) => {
    ...
    const c = createHttpSmartClient({ baseUrl: "https://127.0.0.1/__runner", auth: { header: "x-token", token: "secret" } });
  ...
  it("parseMaybeJsonResponse: rejects when response emits error", async () => {
    jest.spyOn(http, "request").mockImplementation((opts: any, cb: any) => {
      const res = new Readable({ read() { /* no data */ } });
      const im = asIncoming(res, { "content-type": "application/json" });
    ...
    const input = { file: createNodeFile({ name: "x" }, { stream: Readable.from("x") }, "F5") } as const;
    await expect(client.task("upload", input as any)).rejects.toBeTruthy();
  });
  ...
  it("onRequest is invoked for multipart and octet-stream", async () => {
    ...
    const c = createHttpSmartClient({ baseUrl, onRequest });
    await c.task("upload", { file: createNodeFile({ name: "x" }, { stream: Readable.from("x") }, "Fz") } as any);
