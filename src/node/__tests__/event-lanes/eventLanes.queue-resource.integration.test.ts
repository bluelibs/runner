...
describe("event-lanes: queue resource bindings", () => {
  ...
  it("accepts DLQ queue resources in bindings", async () => {
    const lane = r
      .eventLane("tests.event-lanes.queue-resource.dlq.lane")
      .build();
    const event = r
      .event<{ id: string }>("tests.event-lanes.queue-resource.dlq.event")
      .tags([globals.tags.eventLane.with({ lane })])
      .build();

    let hookCalls = 0;
    const hook = r
      .hook("tests.event-lanes.queue-resource.dlq.hook")
      .on(event)
      .run(async () => {
        hookCalls += 1;
      })
    ...
    const emitTask = r
      .task("tests.event-lanes.queue-resource.dlq.emit")
      .dependencies({ event })
      .run(async (_input, deps) => {
        await deps.event({ id: "1" });
      })
    ...
    const mainQueue = new ResourceBackedQueue();
    const dlqResource = r
      .resource("tests.event-lanes.queue-resource.dlq.queue")
      .init(async () => new ResourceBackedQueue())
      .dispose(async (queue) => {
        await queue.dispose();
      })
    ...
    const app = r
      .resource("tests.event-lanes.queue-resource.dlq.app")
      .register([
        event,
        hook,
        emitTask,
        ...
        eventLanesResource.with({
          ...
          topology: {
            profiles: { worker: { consume: [lane] } },
            bindings: [{ lane, queue: mainQueue, dlq: { queue: dlqResource } }],
          },
        }),
      ])
      .build();
    ...
    const runtime = await run(app);
    const dlqQueue = await runtime.getResourceValue(dlqResource);

    await runtime.runTask(emitTask);
    await waitUntil(() => hookCalls === 1);

    await runtime.dispose();
    expect(mainQueue.disposeCalls).toBe(1);
    expect(dlqQueue.disposeCalls).toBe(1);
  });
});
