...
describe("durable: RecoveryManager (unit)", () => {
  ...
  it("handles no-lock, missing renewLock, renewal retries, and stopped heartbeats", async () => {
    const { manager } = createManager();

    const noopStop = (manager as any).startClaimHeartbeat(
    ...
    const noRenewManager = new RecoveryManager(
      {
    ...
    const noRenewStop = (noRenewManager as any).startClaimHeartbeat(
    ...
    const stoppableManager = new RecoveryManager(
      {
    ...
    const stop = (stoppableManager as any).startClaimHeartbeat(
