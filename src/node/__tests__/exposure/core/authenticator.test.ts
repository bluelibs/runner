  ...
  describe("createAuthenticator", () => {
    it("returns AUTH_NOT_CONFIGURED when token is not set and no validators (fail-closed)", async () => {
      ...
      if (!result.ok) {
        ...
        expect((result.response.body as unknown as { error: { code: string } }).error.code).toBe("AUTH_NOT_CONFIGURED");
      }
    });
    ...
    it("accepts provided token using custom header and array values", async () => {
      ...
      const ok = await auth({ headers: { "x-custom": ["secret"] } } as unknown as IncomingMessage);
    ...
    it("accepts provided token from default header string", async () => {
      ...
      const ok = await auth({ headers: { "x-runner-token": "expected" } } as unknown as IncomingMessage);
      expect(ok).toEqual({ ok: true });
    });
    ...
    it("falls back to empty string when header array has no first value", async () => {
      const auth = createAuthenticator({ token: "expected" }, mockTaskRunner, []);
      const result = await auth({ headers: { "x-runner-token": [] } } as unknown as IncomingMessage);
      expect(result.ok).toBe(false);
    });

    it("supports array of tokens", async () => {
      const auth = createAuthenticator({ token: ["token1", "token2"] }, mockTaskRunner, []);
      const ok1 = await auth({ headers: { "x-runner-token": "token1" } } as unknown as IncomingMessage);
      expect(ok1).toEqual({ ok: true });
      const ok2 = await auth({ headers: { "x-runner-token": "token2" } } as unknown as IncomingMessage);
      expect(ok2).toEqual({ ok: true });
      const fail = await auth({ headers: { "x-runner-token": "wrong" } } as unknown as IncomingMessage);
    ...
    it("runs validator tasks when token check fails", async () => {
      ...
      const result = await auth({ headers: {} } as unknown as IncomingMessage);
      expect(mockTaskRunner.run).toHaveBeenCalledWith(task, expect.objectContaining({ url: "/", method: "GET" }));
      expect(result).toEqual({ ok: true });
    });
