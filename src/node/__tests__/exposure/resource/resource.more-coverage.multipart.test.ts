...
import { makeReqRes, createBaseReq, MockRes } from "./resource.more-coverage.test.utils";

describe("nodeExposure - more multipart coverage", () => {
  it("multipart: unknown file field triggers stream.resume() path", async () => {
    const echo = defineTask<{ n: number }, Promise<number>>({ id: "exposer.more.echo", run: async ({ n }) => n });
    ...
    const app = defineResource({ id: "exposer.more.app1", register: [echo, exposure] });
    ...
    const { req, res } = makeReqRes(body, { "x-runner-token": "T", "content-type": `multipart/form-data; boundary=${boundary}`, "content-length": String(Buffer.byteLength(body)) });
  ...
  it("multipart: file part with empty name is ignored (stream.resume path)", async () => {
    const echo = defineTask<{ n: number }, Promise<number>>({ id: "exposer.more.emptyname", run: async ({ n }) => n });
    const exposure = nodeExposure.with({ http: { dangerouslyAllowOpenExposure: true, server: http.createServer(), basePath: "/__runner", auth: { token: "T" } } });
    const app = defineResource({ id: "exposer.more.app1b", register: [echo, exposure] });
    const rr = await run(app);
    const handlers = await rr.getResourceValue(exposure.resource);
    ...
    const ref = makeReqRes(body, { "x-runner-token": "T", "content-type": `multipart/form-data; boundary=${boundary}`, "content-length": String(Buffer.byteLength(body)) });
  ...
  it("multipart: request stream error triggers 499 (busboy error path)", async () => {
    const fileTask = defineTask<{ name: string }, Promise<string>>({ id: "exposer.more.busboy.error", run: async ({ name }) => name });
    const exposure = nodeExposure.with({ http: { dangerouslyAllowOpenExposure: true, server: http.createServer(), basePath: "/__runner", auth: { token: "T" } } });
    const app = defineResource({ id: "exposer.more.app6", register: [fileTask, exposure] });
    const rr = await run(app);
    const handlers = await rr.getResourceValue(exposure.resource);
    ...
    req.headers = { "x-runner-token": "T", "content-type": `multipart/form-data; boundary=${boundary}`, "content-length": "0" };
    ...
    const out = payload ? JSON.parse((payload as Buffer).toString("utf8")) : undefined;
