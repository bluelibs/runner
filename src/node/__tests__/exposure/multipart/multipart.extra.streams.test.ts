...
import { createMockRequest, expectErrorCode, type FakeStream } from "./multipart.mock.utils";
...
describe("parseMultipartInput - streams extra branches", () => {
  ...
  it("fails when file handler hits files limit and resumes the stream", async () => {
    ...
    const req = createMockRequest(baseHeaders, (busboy) => {
      const mkStream = () => ({ on() { return this; }, pipe() {}, resume: resumeSpy });
      busboy.emit("file", "file:F1", mkStream(), { filename: "a.txt", mimeType: "text/plain" });
      busboy.emit("file", "file:F2", mkStream(), { filename: "b.txt", mimeType: "text/plain" });
    });
  ...
  it("falls back to ending streams if destroy throws", async () => {
    ...
    let ended = false;
    PassThrough.prototype.destroy = function () { throw new Error("nope"); };
    ...
    try {
      const req = createMockRequest(baseHeaders, (busboy) => {
        const manifest = JSON.stringify({ input: { f: { $runnerFile: "File", id: "F1" } } });
