...
import { createMultipartRequest, part, expectErrorCode, createErroringRequest, createRequestFromBody } from "./multipart.test.utils";
...
describe("parseMultipartInput - Errors", () => {
  ...
  it("fails when manifest is missing", async () => {
    ...
    if (parsed.ok) throw new Error("Expected multipart failure for missing manifest");
    expectErrorCode(parsed.response, "MISSING_MANIFEST");
  });

  it("fails when manifest JSON is invalid", async () => {
    const req = createMultipartRequest(boundary, [
      part(boundary, ['Content-Disposition: form-data; name="__manifest"', "Content-Type: application/json; charset=utf-8"], "not-json"),
    ]);
    const parsed = await parseMultipartInput(req, undefined, serializer);
    if (parsed.ok) throw new Error("Expected multipart failure for invalid manifest");
  ...
  it("propagates request stream errors", async () => {
    ...
    if (parsed.ok) {
      ...
      if (finalize.ok) throw new Error("Expected finalize to report request abort");
  ...
  it("reports multipart parser errors (missing boundary)", async () => {
    const req = createRequestFromBody("irrelevant", { "content-type": "multipart/form-data" });
    const parsed = await parseMultipartInput(req, undefined, serializer);
    if (parsed.ok) {
      const finalize = await parsed.finalize;
      if (finalize.ok) throw new Error("Expected missing boundary to be treated as invalid");
  ...
  it("finalize surfaces missing file part errors", async () => {
    const manifest = JSON.stringify({ input: { file: { $runnerFile: "File", id: "F1", meta: { name: "late.txt" } } } });
    const req = createMultipartRequest(boundary, [
    ...
    const finalize = await parsed.finalize;
    if (finalize.ok) throw new Error("Expected finalize to report missing file part");
