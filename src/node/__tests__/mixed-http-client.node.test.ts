...
describe("createMixedHttpClient (unit)", () => {
  ...
  it("JSON path: uses fetch client for plain inputs", async () => {
    ...
    const fetchMock = async (url: any, init?: any) => {
      ...
      calls.push({ url: String(url), headers: init?.headers ?? {}, body: JSON.parse(bodyStr) });
      return { text: async () => getDefaultSerializer().stringify({ ok: true, result: 42 }) } as any;
    };

    const client = createMixedHttpClient({ baseUrl, fetchImpl: fetchMock as any });
  ...
  it("Readable input: delegates to Smart client and returns a stream", async () => {
    ...
    const client = createMixedHttpClient({ baseUrl });
  ...
  it("Node File sentinel: delegates to Smart client (multipart)", async () => {
    ...
    const client = createMixedHttpClient({ baseUrl });
    const input = { file: createNodeFile({ name: "a.txt" }, { stream: Readable.from("abc") }, "F1") } as const;
  ...
  it("Array + nested object sentinel: Smart client path is used", async () => {
    ...
    const client = createMixedHttpClient({ baseUrl });
    const input = { arr: [{ nested: createNodeFile({ name: "n.bin" }, { buffer: Buffer.from([1,2,3]) }, "FN") }] } as const;
    const out = await client.task("upload", input as any);
    expect(out).toBe("OK2");
    expect(reqSpy).toHaveBeenCalled();
  });
  ...
  it("event(): always uses JSON path", async () => {
    ...
    const fetchMock = async (url: any, init?: any) => {
      ...
      return { text: async () => getDefaultSerializer().stringify({ ok: true, result: undefined }) } as any;
    };
  ...
  it("throws when baseUrl is empty", () => {
    expect(() => createMixedHttpClient({ baseUrl: "" as any })).toThrow();
  });
});
