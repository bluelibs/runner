...
export function CrashControl({ executionId }: CrashControlProps) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<EditStateForm>({
    defaultValues: {
      stepId: '',
          ...
          <form onSubmit={handleSubmit(onEditState)} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <textarea
                  {...register('newStateJson', { required: true, validate: (v) => {
            ...
            <button type="submit" disabled={loading} className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-medium rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50">
