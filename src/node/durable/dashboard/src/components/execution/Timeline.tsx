...
export type TimelineStep = {
  id: string;
  name: string;
  status: 'completed' | 'running' | 'failed' | 'compensation_failed' | 'pending';
  duration?: string;
  errorMessage?: string;
};
...
const StatusIcon = ({ status, className }: any) => {
  switch (status) {
    ...
    case 'running': return <Clock className={`text-blue-500 animate-spin ${className}`} />;
    case 'failed': return <XCircle className={`text-red-500 ${className}`} />;
    case 'compensation_failed': return <AlertOctagon className={`text-pink-500 animate-pulse ${className}`} />;
    default: return <div className={`w-5 h-5 rounded-full border-2 border-slate-700 ${className}`} />;
  }
};
        ...
        <div key={step.id} className="relative flex items-start group">
          ...
          <div className="ml-6 flex-1 bg-slate-900 border border-slate-800 rounded-lg p-4 group-hover:border-slate-700 transition-colors cursor-pointer">
            <div className="flex items-center justify-between">
              <h4 className={`font-mono font-medium ${step.status === 'compensation_failed' ? 'text-pink-500' : 'text-slate-200'}`}>
                {step.name}
              </h4>
            ...
            {step.status === 'compensation_failed' && (
               <div className="mt-3 text-sm text-pink-400 bg-pink-500/5 p-2 rounded border border-pink-500/10">
                 Critical Error: Compensation logic failed. Manual intervention required.
               </div>
            )}
          </div>
        </div>
