...
import { Activity, CheckCircle2, AlertOctagon, Clock, RefreshCw, Power } from 'lucide-react';
...
const COLORS: Record<string, string> = {
  ...
  compensation_failed: '#ec4899', // pink-500
  pending: '#eab308', // yellow-500
  sleeping: '#a855f7', // purple-500
...
export function Dashboard() {
  ...
  const stats = {
      ...
      active: executions.filter(e => ['running', 'pending', 'retrying', 'sleeping'].includes(e.status)).length,
      completed: executions.filter(e => e.status === 'completed').length,
      critical: executions.filter(e => e.status === 'compensation_failed' || e.status === 'failed').length,
  };

  const chartData = Object.entries(COLORS).map(([status, color]) => ({
      name: status,
  ...
  const recentAlerts = executions
    .filter(e => e.status === 'compensation_failed' || e.status === 'failed')
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-2">Durable Engine Status: <span className="text-green-500 font-mono">ONLINE</span></p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => fetchData()} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-200 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="Total Executions" value={stats.total} icon={Activity} color="bg-blue-500 text-blue-500" />
        <StatCard title="Active" value={stats.active} icon={Clock} color="bg-yellow-500 text-yellow-500" />
        <StatCard title="Completed" value={stats.completed} icon={CheckCircle2} color="bg-green-500 text-green-500" />
        <StatCard title="Failures" value={stats.critical} icon={AlertOctagon} color="bg-pink-500 text-pink-500" />
      </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <div className="space-y-4">
            {recentAlerts.length === 0 ? (
              ...
              <Link to={`/executions/${e.id}`} key={e.id} className="block group">
                  <div className="flex items-start gap-4 p-4 rounded-lg bg-pink-500/10 border border-pink-500/20 group-hover:bg-pink-500/20 transition-colors">
                    ...
                    <div>
                    <h4 className="font-medium text-pink-500 capitalize">{e.status.replace('_', ' ')}</h4>
                    <p className="text-sm text-slate-400 mt-1">Execution {e.id}</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <div className="overflow-x-auto">
                          ...
                          <tr key={e.id} className="hover:bg-slate-800/50 transition-colors">
                              ...
                              <td className="p-4">
                                  <span className={`px-2 py-1 rounded text-xs font-medium border ${
                                      ...
                                      e.status === 'completed' ? 'bg-green-500/10 text-green-500 border-green-500/20' :
                                      e.status === 'failed' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                                      e.status === 'compensation_failed' ? 'text-pink-500 border-pink-500/20 bg-pink-500/10' :
                              ...
                              <td className="p-4">
                                  <Link to={`/executions/${e.id}`} className="text-blue-400 hover:text-blue-300 hover:underline">
                                      View
                                  </Link>
                              </td>
                          </tr>
              ...
              {executions.length === 0 && !loading && (
                  <div className="p-8 text-center text-slate-500">No executions found.</div>
              )}
          </div>
      </div>
    </div>
