...
const createPointType = (): TypeDefinition<
  Point,
  { x: number; y: number }
> => ({
  id: "Point",
...
> => ({
  ...
  serialize: (complex: ComplexNumber) => ({
    real: complex.real,
...
> => ({
  id: "LinkedListNode",
  is: (obj: unknown): obj is LinkedListNode<unknown> =>
    obj instanceof LinkedListNode,
  ...
  deserialize: (data: {
    value: unknown;
    next: LinkedListNode<unknown> | null;
...
describe("Custom Type Tests", () => {
  ...
  describe("LinkedListNode Type", () => {
    it("should handle singly linked list with custom type", () => {
      ...
      const deserialized =
        serializer.deserialize<LinkedListNode<string>>(serialized);
    ...
    it("should handle doubly linked list with circular references", () => {
      ...
      if (!nextNode) {
        throw genericError.new({ message: "Expected next node to be defined" });
      }
  ...
  describe("Multiple Custom Types", () => {
    it("should handle objects with multiple custom types", () => {
      ...
      const deserialized =
        serializer.deserialize<MixedCustomStructure>(serialized);
