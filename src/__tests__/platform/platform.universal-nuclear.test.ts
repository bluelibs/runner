describe.skip("UniversalPlatformAdapter - THE MATRIX (skipped: brittle prototype overrides and TS constraints)", () => {
  it("should break reality itself to achieve 100% coverage", async () => {
    ...
    try {
      ...
      const originalInit = universalModule.UniversalPlatformAdapter.prototype.init;
      const originalGet = universalModule.UniversalPlatformAdapter.prototype.get;
      
      // Replace init method to force our scenario
    universalModule.UniversalPlatformAdapter.prototype.init = async function() {
      ...
      const kind: any = "browser";
          ...
          if (typeof (globalThis as any).document !== "undefined" || typeof (globalThis as any).addEventListener === "function") {
            ...
            switch (kind) {
              case "node":
                (this as any).inner = new (require("../../platform/adapters/node").NodePlatformAdapter)();
                break;
              case "browser":
                ...
                break;
              case "edge":
                (this as any).inner = new (require("../../platform/adapters/edge").EdgePlatformAdapter)();
                break;
              default:
                (this as any).inner = new (require("../../platform/adapters/universal-generic").GenericUniversalPlatformAdapter)();
            }
          }
        }
