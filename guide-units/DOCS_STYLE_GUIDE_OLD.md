...
- Use emoji for major landmark sections (Quick Wins, Reference)
- Don't use emoji in core concept headings (Tasks, Resources)
- Use emoji consistently across similar sections
...
Good with emoji (special sections):
...
1. **Title** (with optional emoji for Quick Wins/Reference sections)
...
```typescript
//  Simple examples - inline validation
.inputSchema<{ name: string }>({ parse: (v) => v })

//  Real-world examples - reference Zod
import { z } from "zod";
...
**Checklist pattern:**
Use checkmarks for achievement lists:
...
**Conditional lists:**
Use emojis to indicate when to use certain approaches:
...
Use sparingly and consistently. Emojis serve as visual anchors for quick scanning.

**Core Usage:**

| Emoji | Purpose                    | Example                            |
| ----- | -------------------------- | ---------------------------------- |
|       | Good practice, recommended | `//  Good - explicit imports`      |
|       | Bad practice, avoid        | `//  Bad - assumes globals`        |
| ️      | Warning, partial support   | `️ **Platform Note**: Node.js only` |

**Section Markers:**

| Emoji | Purpose                          | Example                                |
| ----- | -------------------------------- | -------------------------------------- |
|       | Quick wins, actionable solutions | `##  Quick Wins: Copy-Paste Solutions` |
|       | Reference, documentation         | `##  Quick Reference Cheat Sheet`      |
|       | Troubleshooting, tools           | `##  Troubleshooting`                  |
|       | Feature highlights, "magic"      | `##  Show Me the Magic`                |
|       | Comparisons, metrics             | `##  How Does It Compare?`             |
|       | Performance, speed               | `##  Performance at a Glance`          |
|       | Features, what's included        | `##  What's in the Box?`               |

**Contextual Usage:**

| Emoji | Purpose                  | Example                      |
| ----- | ------------------------ | ---------------------------- |
|       | Testing                  | Testing patterns, unit tests |
|       | Debugging                | Debug sections               |
| ️      | Patterns, tags           | Tagging patterns             |
|       | Type helpers, inspection | TypeScript utilities         |
| 🪶    | Lightweight, minimal     | Bundle size comparisons      |
|       | Heavy, large             | Framework size comparisons   |
|       | Medium size              | Package comparisons          |
|       | Launch, deploy, fast     | Production deployment        |
|       | Cost, resources          | Cloud costs                  |
| ️      | Steep, difficult         | Learning curve               |

**Numbered Sequences:**

Use number emojis for sequential steps in "Quick Wins" or tutorials:

```markdown
### Add Caching to Any Task
...
- Use emojis consistently across similar sections
- Limit to 1-2 emojis per heading
- Prefer standard emojis over custom Unicode
- Don't overuse - they should enhance, not clutter
...
| -------------------- | ------ | ------ | ----------- | -------- |
| **Learning Curve**   | Gentle | ️ Steep | ️ Steep      | Moderate |
...
- Use emojis + text for visual impact
...
> ️ **Platform Note**: `AsyncLocalStorage` is a **Node.js-only** feature.
```

**Types of callouts:**

- `> **runtime:**` - Personality quotes (see Runtime Character section)
- `> ️ **Platform Note:**` - Platform-specific warnings
...
### Problem Name (with result description)
...
Use emoji + text format for visual impact:

```markdown
| Feature            | Runner | NestJS | Other    |
| ------------------ | ------ | ------ | -------- |
| **Learning Curve** | Gentle | ️ Steep | Moderate |
```

**Standard emoji patterns:**

- Learning curve: Gentle, Moderate, ️ Steep
- Size: Small, Medium, Large
- Support: Yes/Full, ️ Partial, No/None
- Speed: Fast, OK, Slow
...
- Use emoji bullets for each benefit
- Bold the key phrase
...
```markdown
> ️ **Platform Note**: `AsyncLocalStorage` is a **Node.js-only** feature. In browsers and edge runtimes, async context is not available.
```
