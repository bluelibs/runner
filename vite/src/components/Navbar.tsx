...
const Navbar: React.FC<NavbarProps> = ({ darkMode, toggleDarkMode }) => {
  ...
  return (
    <nav className="fixed w-full z-50 backdrop-blur-lg border-b" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 backdrop-blur-lg border-t" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
