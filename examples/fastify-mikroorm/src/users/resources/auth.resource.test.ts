...
describe("auth resource", () => {
  ...
  it("signs and verifies tokens, handles expiry and parse errors, cookie flags", async () => {
    ...
    try {
      ...
      const h = Buffer.from(JSON.stringify(header)).toString("base64").replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
      const p = Buffer.from("not-json").toString("base64").replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
