...
describe("auth resource", () => {
  ...
  it("handles token verification edge cases", async () => {
    const rr = await buildTestRunner({ register: [auth.with({ secret: "test" })] });
    try {
      const a = rr.getResourceValue(auth);
  ...
  it("covers b64url padding edge cases in token verification", async () => {
    ...
    try {
      ...
      const testCases = [
        ...
        { sub: "very-long-user-id-string", exp: Math.floor(Date.now() / 1000) + 60 },
        ...
        { sub: "u", exp: Math.floor(Date.now() / 1000) + 60, additional: "field", more: "content" },
      ];
    ...
    try {
      ...
      const authWithoutEnv = resource({
        ...
        init: async (cfg: AuthConfig, { logger }): Promise<AuthValue> => {
          ...
          const isProd = (envValue?.NODE_ENV || process.env.NODE_ENV) === 'production';
