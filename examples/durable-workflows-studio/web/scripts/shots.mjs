...
import { captureScaleShots } from "./scale-shot.mjs";
...
const { chromium } = await import("playwright-core").catch(() => {
  console.error("playwright-core is missing. Run `npm install` in web/ first.");
  process.exit(1);
});
...
try {
  ...
  if (process.env.SHOTS_SCALE_ONLY !== "1") {
    ...
    await shot(
      "06-signal-modal.png",
      "?demo=1&select=demo_inc_live&modal=signal",
    );
    ...
    assert.ok(initialWorkflowCount > 0 && initialWorkflowCount < 35);
    ...
    assert.ok((await workflowRows.count()) < 35);
    await page
      .getByRole("button", { name: "Case escalation risk", exact: true })
      .waitFor();
    await page.screenshot({ path: join(shotsDir, "18-workflow-scroll.png") });
    console.log(
      "saved 18-workflow-scroll.png; workflow scroll and dot-free labels verified",
    );
    ...
    await page
      .locator(".sidebar")
      .getByLabel("Search workflows")
      .fill("case escalation");
    await page
      .getByRole("button", { name: "Case escalation risk", exact: true })
      .waitFor();
