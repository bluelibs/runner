/**
 * Static studio catalog: workflow metadata, visualisation graphs, input and
 * signal presets.
 *
 * Durable workflows are code, not declarative graphs, so the studio overlays
 * runtime state on a hand-maintained static graph per workflow. Graph node ids
 * must equal the persisted durable step ids (`__sleep:` / `__signal:`
 * prefixes included) — `workflows.test.ts` guards this contract.
 */
import type { StudioSignal, StudioWorkflow } from "../shared/types.js";
import {
  ApprovalDecision,
  EmailVerified,
...
import { OPERATIONS_WORKFLOWS } from "./operationsCatalog.js";

export const SIGNALS: Record<string, StudioSignal> = {
  [PaymentConfirmed.id]: {
    ...
    presets: [
      {
        name: "Card settled",
        payload: { transactionId: "txn_demo_123" },
      },
    ],
  },
  [EmailVerified.id]: {
  ...
  [IncidentAcknowledged.id]: {
    id: IncidentAcknowledged.id,
...
export const WORKFLOWS: StudioWorkflow[] = [
  {
    key: "processOrder",
    ...
    signals: [SIGNALS[PaymentConfirmed.id]!],
    presets: [
      {
        name: "Standard order",
        payload: {
  ...
  },
  {
    ...
    signals: [SIGNALS[EmailVerified.id]!],
    presets: [
      {
        name: "Pro trial",
  ...
  },
  {
    ...
    signals: [
      SIGNALS[IncidentAcknowledged.id]!,
      SIGNALS[ApprovalDecision.id]!,
    ],
    presets: [
      {
        name: "SEV-2 outage",
    ...
    graph: {
      nodes: [
        ...
        },
        {
          ...
          signal: IncidentAcknowledged.id,
        },
        {
          id: "ackBranch",
          kind: "switch",
...
export function getWorkflow(key: string): StudioWorkflow | undefined {
