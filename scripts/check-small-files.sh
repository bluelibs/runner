...
# For small-files: use files with count < |threshold|. Negative values are accepted.
...
if [[ -n "$THRESHOLD_FROM_ARG" ]]; then
  if [[ "$THRESHOLD_FROM_ARG" =~ ^-?[0-9]+$ ]]; then
    ...
    echo "Invalid threshold: $THRESHOLD_FROM_ARG (must be an integer, negative allowed)" >&2
    exit 2
  fi
  ...
  if [[ "$THRESHOLD_FROM_ENV" =~ ^-?[0-9]+$ ]]; then
    ...
    echo "Invalid THRESHOLD env var: $THRESHOLD_FROM_ENV (must be an integer, negative allowed)" >&2
    exit 2
  fi
else
...
# Use absolute value for cutoff; small-files means: count < cutoff
CUTOFF="${THRESHOLD#-}"
...
if [[ ${#results[@]} -eq 0 ]]; then
  echo "No files under $CUTOFF non-empty lines found under $SRC_DIR."
  exit 0
fi

# Sort by non-empty line count asc and print
